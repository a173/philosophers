...
/*   Updated: 2021/04/09 15:45:21 by acaryn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_struct(t_all *all, t_philo *philo, int i)
{
	...
	if (!philo->left || pthread_mutex_init(philo->left, NULL))
		return (0);
	return (1);
}
...
static int	init_array(t_all *all, t_philo **philo, int i)
{
	while (++i < all->count_philo)
	{
		...
		if (!philo[i] || !init_struct(all, philo[i], i))
			return (0);
		if (i)
			philo[i - 1]->right = philo[i]->left;
	}
	philo[i - 1]->right = philo[0]->left;
	philo[i] = NULL;
	return (1);
}
...
static int	parse(char **argv, t_all *all, int i, int j)
{
	...
	if (!all->print || pthread_mutex_init(all->print, NULL))
		return (0);
	if (argv[4])
